...
parser = argparse.ArgumentParser(
    description="Runs few-shot trigger sequence labeling training on source dataset with inference on source"
)
...
parser.add_argument(
    ...
    type=str,
    help="The test dataset for source domain inference",
)
...
if __name__ == "__main__":
    ...
    source_dataset = dataset_mapping[config.dataset_to_train_and_few_shot_evaluate_on]

    few_shot_train = source_dataset(
        fpath=config.dataset_train_few_shot_path,
        tokenizer=tokenizer,
        trigger2id=label2id,
        task=config.training_task,
    )
    few_shot_test = source_dataset(
        fpath=config.dataset_test_few_shot_path,
        tokenizer=tokenizer,
        trigger2id=label2id,
        task=config.training_task,
    )
    ...
    for i, few_shot_dataset in enumerate(few_shot_samples_train):
        ...
        all_metrics_few_shot = train_evaluate(
            model=bert_model,
            loader_few_shot_train=loader_few_shot_train,
        ...
        for key_average in [
            x for x in all_metrics_few_shot["identification"].keys() if "overall" in x
        ]:
