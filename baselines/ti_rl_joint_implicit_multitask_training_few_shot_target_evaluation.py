...
from util import build_vocab, compute_metrics, logging

from models import RobertaForTriggerRelationTokenClassification
...
parser = argparse.ArgumentParser(
    description="Runs few-shot trigger sequence labeling training on source dataset with inference on target using pretrained model"
)
...
parser.add_argument(
    "--freeze_embs",
    type=parse_boolean,
    help="Update pretrained embeddings during training or not",
)
parser.add_argument(
    "--label_hidden_size",
    type=int,
...
def train(
    ...
    for i in tqdm(range(epochs)):
        ...
        few_shot_train_all_metrics = compute_metrics(model, loader_few_shot_train)

        valid_all_metrics = logging(
            model=model,
            loader=loader_valid,
            loss=valid_loss,
            epoch=i,
        ...
        train_metrics = {
            ...
            + configuration_string: few_shot_train_all_metrics["identification"][
                "overall_f1"
            ],
            "epoch": i,
        }
...
def calculate_epoch_loss(
    ...
    if mode == "train":
        ...
        for batch in loader:
            ...
            few_shot_loss = collect_losses(model, criterion, few_shot_batch)

            loss_1 += tag_loss.item()
            loss_2 += few_shot_loss.item()

            avg_loss = (
                tag_loss / batch["labels"].shape[0]
                + few_shot_loss / few_shot_batch["labels"].shape[0]
            ) / 2
        ...
        loss = (loss_1 / len(loader.dataset) + loss_2 / total_few_shot_examples) / 2
    else:
        model.eval()
...
def train_evaluate(
    ...
    all_metrics_few_shot = compute_metrics(model, loader_few_shot_test)

    wandb.summary["valid_f1 " + configuration_string] = best_valid_score
    wandb.summary["test_all_metrics " + configuration_string] = all_metrics
    wandb.summary[
        "test_all_metrics_few_shot " + configuration_string
    ] = all_metrics_few_shot

    print("test_all_metrics", configuration_string, all_metrics)
    print("test_all_metrics_few_shot", configuration_string, all_metrics_few_shot)

    return all_metrics, all_metrics_few_shot
...
if __name__ == "__main__":
    ...
    test_metrics_averaged = defaultdict(float)
    ...
    for i, few_shot_dataset in enumerate(few_shot_samples_train):
        ...
        if config.model != config.load_pretrained_model_from:
            bert_model = RobertaForTriggerRelationTokenClassification.from_pretrained(
        ...
        if config.freeze_embs:
            bert_model.label_type_embeddings.weight.requires_grad = False
        ...
        all_metrics, all_metrics_few_shot = train_evaluate(
            model=bert_model,
            loader_train=loader_train,
            loader_valid=loader_valid,
        ...
        for key_average in [
            x for x in all_metrics["identification"].keys() if "overall" in x
        ]:
            test_metrics_averaged[key_average + "_averaged"] += all_metrics[
                "identification"
            ][key_average]
            test_metrics_few_shot_averaged[
                key_average + "_averaged"
            ] += all_metrics_few_shot["identification"][key_average]

    test_metrics_averaged = {
        k: v / config.average_over_runs_num for k, v in test_metrics_averaged.items()
    }
    test_metrics_few_shot_averaged = {
    ...
    wandb.summary["test_all_metrics_averaged"] = test_metrics_averaged
    wandb.summary["test_all_metrics_few_shot_averaged"] = test_metrics_few_shot_averaged

    print("test_all_metrics_averaged", test_metrics_averaged)
    print("test_all_metrics_few_shot_averaged", test_metrics_few_shot_averaged)
