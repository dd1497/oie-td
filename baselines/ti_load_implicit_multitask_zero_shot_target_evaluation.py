...
from util import build_vocab, compute_metrics
...
parser.add_argument(
    "--dataset_target_mini_extractions_path",
    type=str,
...
if __name__ == "__main__":
    ...
    _, id2trigger, trigger2id = build_vocab(["Trigger"])
    all_labels_relation, all_labels_trigger, relation2id = build_vocab(["Relation"])
    ...
    zero_shot_test = target_dataset(
        fpath_trigger=config.dataset_target_to_evaluate_on_path,
        fpath_relation=config.dataset_target_mini_extractions_path,
        tokenizer=tokenizer,
