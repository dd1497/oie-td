...
from models import RobertaForMultiTaskOneHeadTokenClassification
...
parser.add_argument(
    "--dataset_valid_pretrained_path",
    type=str,
    help="The source valid dataset that the pretrained model was evaluated on",
)
parser.add_argument(
    "--dataset_test_pretrained_path",
    type=str,
    help="The source test dataset that the pretrained model was evaluated on",
)
parser.add_argument(
    "--dataset_valid_pretrained_mini_extractions_path",
    type=str,
    help="The valid relation dataset that the pretrained model was evaluated on",
)
parser.add_argument(
    "--dataset_test_pretrained_mini_extractions_path",
    type=str,
    help="The test relation dataset that the pretrained model was evaluated on",
)
...
def train_evaluate(
    ...
    all_metrics_test = compute_metrics_multitask(model, loader_test, "labels")
    ...
    wandb.summary["test_all_metrics_trigger " + configuration_string] = all_metrics_test
    wandb.summary[
        "test_all_metrics_few_shot_trigger " + configuration_string
    ] = all_metrics_few_shot_test

    print("test_all_metrics_trigger", configuration_string, all_metrics_test)
    print(
    ...
    return all_metrics_test, all_metrics_few_shot_test
...
if __name__ == "__main__":
    ...
    source_dataset = dataset_mapping[config.dataset_the_model_was_pretrained_on]
    target_dataset = dataset_mapping[config.dataset_to_train_and_few_shot_evaluate_on]

    if config.dataset_the_model_was_pretrained_on == "MAVEN":
        # here actually needs to be passed path to maven train in the training script
        dataset_train = source_dataset(
            fpath_trigger=config.dataset_valid_pretrained_path,
            fpath_relation=config.dataset_valid_pretrained_mini_extractions_path,
            tokenizer=tokenizer,
    ...
    dataset_test = source_dataset(
        fpath_trigger=config.dataset_test_pretrained_path,
        fpath_relation=config.dataset_test_pretrained_mini_extractions_path,
        tokenizer=tokenizer,
        trigger2id=trigger2id,
        relation2id=relation2id,
    )
    ...
    for i, few_shot_dataset in enumerate(few_shot_samples_train):
        ...
        all_metrics_test, all_metrics_few_shot_test = train_evaluate(
            model=bert_model,
            loader_valid=loader_valid,
        ...
        for key_average in [
            ...
            test_metrics_averaged[key_average + "_averaged"] += all_metrics_test[
                "identification"
            ][key_average]
    ...
    wandb.summary["test_all_metrics_trigger_averaged"] = test_metrics_averaged
    wandb.summary[
        "test_all_metrics_few_shot_trigger_averaged"
    ] = test_metrics_few_shot_averaged

    print("test_all_metrics_trigger_averaged", test_metrics_averaged)
