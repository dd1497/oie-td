...
from models import RobertaForMLMMultiTaskTwoHeadsTokenClassification
...
parser = argparse.ArgumentParser(
    description="Runs trigger and relation sequence labeling training on source dataset with MLM validation and inference on target"
)
...
def train(
    ...
    for i in tqdm(range(epochs)):
        train_loss, _ = calculate_epoch_loss(
            model=model,
            loader=loader_train,
            loader_mlm=None,
            optimizer=optimizer,
            criterion=criterion,
            criterion_mlm=criterion_mlm,
            mode="train",
        )
        ...
        train_all_metrics_trigger = compute_metrics_multitask(
            model, loader_train, "labels"
        )
        train_all_metrics_relation = compute_metrics_multitask(
            model, loader_train, "label_type_ids"
        )

        valid_all_metrics_trigger = compute_metrics_multitask(
            model, loader_valid, "labels"
        )
        valid_all_metrics_relation = compute_metrics_multitask(
            model, loader_valid, "label_type_ids"
        )

        valid_perp = get_perplexity(valid_losses_mlm)
        ...
        if valid_perp < best_valid_score:
            best_model = copy.deepcopy(model.state_dict())
            best_valid_score = valid_perp
    ...
    if mode == "train":
        ...
        with torch.no_grad():
            for batch in loader_mlm:
                batch = {k: v.to(model.device) for k, v in batch.items()}
                mlm_loss = collect_losses_mlm(model, criterion_mlm, batch)
...
def train_evaluate(
    ...
    wandb.summary["valid_perp"] = best_valid_score
...
if __name__ == "__main__":
    ...
    bert_model = RobertaForMLMMultiTaskTwoHeadsTokenClassification.from_pretrained(
        config.model,
