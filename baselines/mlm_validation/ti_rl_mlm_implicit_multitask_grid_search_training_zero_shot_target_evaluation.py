...
from util import CustomCollatorMLM, build_vocab, compute_metrics, get_perplexity
...
parser = argparse.ArgumentParser(
    description="Runs trigger sequence labeling training with MLM validation with the help of relation mini extractions on source dataset with inference on target"
)
...
def train(
    ...
    for i in tqdm(range(epochs)):
        ...
        train_all_metrics = compute_metrics(model, loader_train)
        valid_all_metrics = compute_metrics(model, loader_train)
...
if __name__ == "__main__":
    ...
    best_valid_score_global = math.inf
    ...
    for lr_event_emb in config.lr_event_embs:
        for label_hidden_size in config.label_hidden_sizes:
            ...
            wandb.summary[
                "valid_perp " + configuration_string
            ] = best_valid_score_current
