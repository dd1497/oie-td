...
parser = argparse.ArgumentParser(
    description="Runs trigger sequence labeling training with the help of relation mini extractions on source dataset and saves the model"
)
parser.add_argument("--lr", type=float, help="Learning rate for optimizer")
parser.add_argument(
    "--lr_event_emb",
    type=float,
    help="Learning rate for optimizer for relation tag embeddings",
)
parser.add_argument(
    "--model",
    type=str,
...
if __name__ == "__main__":
    ...
    configuration_string = (
        "(lr_emb="
        + str(config.lr_event_emb)
        + ", hidden_size="
        + str(config.label_hidden_size)
        + ")"
    )
